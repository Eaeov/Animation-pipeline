...
**放好 PV 后，先跑一次扫描** —— 它会自动检测镜头切换点、测量每个镜头
的运动量与主体显著性，**直接给出推荐的切片方案**（可粘贴到 `clips.yaml`）：

```bash
python -m anime_pv.cli scan            # 默认取 assets/source/ 下第一个视频
python -m anime_pv.cli scan --video path/to/pv.mp4 --out scan.json
```
...
【镜头排行 (按换人适配度)】
    ...
    0    0.00   2.50   75.97   0.09   100%    1.4%   210.5
    1    2.50   3.00   73.01   0.03   100%    0.9%    85.7
    3    8.50   3.00    0.00  10.05     0%    0.0% 46524.6   ← 噪点/剧烈运镜, 已排除

【推荐切片方案】
  A: 静止 + 主体清晰  → 建立 pipeline 上限基线
  B: 同规格但运镜不同  → 测跨片段稳定性
  C: 运动最剧烈        → 压力测试, 预期失败
```

> **为什么值得多这一步**：片段选择质量直接决定换人结果。
> 选到主体清晰的静止镜头，成功率高；选到剧烈运镜或空镜，必然失败。
> `scan` 把"数秒数 + 凭感觉挑"变成"看数据挑"。
>
> **局限**：主体检测是**显著性代理**（中心能量 + 饱和度集中 + 对比度分离），
> 不是真正的人脸识别 —— 对"中央有大片高细节背景"会误判。
> 所以结果**只作推荐**，最终仍建议人眼确认关键帧。
...
python -m anime_pv.cli --run-id <run_id> eval --layers L1,L3   # 跳过耗额度的 L2
```
...
> 完整声明见 [`docs/素材来源与授权.md`](docs/素材来源与授权.md)
...
| **PV 来源** | *（待填写：官方 YouTube / B站官方号 / 官网 URL）* |
| **人设图来源** | *（待填写：官方立绘 / 公开素材站，须注明许可）* |
| **使用片段** | A: ?s–?s　B: ?s–?s　C: ?s–?s |

**使用声明**：本项目为技术作业演示，不用于商业用途，
不主张对原素材的任何权利。原始素材不入库（见 `.gitignore`）。
...
```
原始 PV ──▶ [scan]         镜头检测 + 适配度排序 → 推荐切片方案 (辅助步骤)
